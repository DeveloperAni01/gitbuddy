...
async function mergeCommand(branchName) {
    try {
            ...
            if (otherBranches.length === 0) {
                console.log(
                    boxen(
                        ...
                        { padding: 1, borderColor: 'yellow', title: '🤖 GitBuddy Merge', titleAlignment: 'center' }
                    )
                );
                return;
            }
        ...
        if (!allBranches.includes(branchName)) {
            console.log(
                boxen(
                    ...
                    { padding: 1, borderColor: 'red', title: '🤖 GitBuddy Merge', titleAlignment: 'center' }
                )
            );
            return;
        }
        ...
        if (repo.modified.length > 0 ||
            ...
            console.log(
                boxen(
                    ...
                    chalk.gray('  gitbuddy commit "your message"\n\n') +
        ...
        if (repo.branch === 'main' || repo.branch === 'master') {
            ...
            if (!proceed) {
                messages.info('Merge cancelled. Smart choice! 😄', 'GitBuddy Merge');
                return;
            }
        }
        ...
        console.log(
            boxen(
                ...
                { padding: 1, borderColor: 'cyan', title: '🤖 GitBuddy Merge', titleAlignment: 'center' }
            )
        );
        ...
        try {
            ...
            console.log(
                boxen(
                    ...
                    chalk.gray('💡 Tip: Run gitbuddy push to send changes to GitHub!'),
                    { padding: 1, borderColor: 'green', title: '🤖 GitBuddy Merge', titleAlignment: 'center' }
                )
            );
            ...
            if (mergeError.message.includes('CONFLICT') ||
                ...
                console.log(
                    boxen(
                        ...
                        chalk.gray('  5. gitbuddy commit "resolved conflicts"\n\n') +
                        chalk.cyan('To abort this merge:\n') +
                        chalk.gray('  git merge --abort\n\n') +
                        chalk.gray('💡 Tip: Run gitbuddy explain for more help!'),
