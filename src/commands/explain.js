...
// GitBuddy backend server URL
const SERVER_URL = 'https://gitbuddy-server.railway.app';
...
const KNOWN_ERRORS = {
    ...
    'failed to push': {
        ...
        fix: 'Run gitbuddy pull first to get latest changes, then try pushing again.',
    ...
    'already exists': {
        ...
        tip: 'Run gitbuddy branch to see all existing branches!'
    },
    'detached head': {
        explanation: 'You are not on any branch — floating in git history!',
        fix: 'Run: gitbuddy switch main to get back to safety.',
    ...
    'uncommitted changes': {
        ...
        fix: 'Run gitbuddy commit "your message" to save your work.',
    ...
    'rejected': {
        ...
        fix: 'Run gitbuddy pull first, then gitbuddy push again.',
...
async function explainCommand(errorText) {
    try {
        ...
        if (!errorText || errorText.trim() === '') {
            messages.info('No error provided. Nothing to explain!', 'GitBuddy Explain');
            return;
        }
        ...
        console.log(
            boxen(
                ...
                chalk.gray('  3. Run gitbuddy status to see current state\n\n') +
