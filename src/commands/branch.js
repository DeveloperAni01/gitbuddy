...
async function branchCommand(name) {
  try {
    ...
    if (!name) {
      ...
      output += chalk.gray(
        '\n\nTip: gitbuddy branch "feature-name" to create new branch',
      );
      ...
      console.log(
        boxen(
          chalk.yellow(`⚠️  Branch "${formattedName}" already exists!\n\n`) +
            ...
            chalk.gray('Run: gitbuddy switch "' + formattedName + '"'),
          {
            padding: 1,
            borderColor: "yellow",
    ...
    if (repo.modified.length > 0 || repo.untracked.length > 0) {
      ...
      if (action === "cancel") {
        messages.info("Branch creation cancelled.", "GitBuddy Branch");
        return;
      }
      ...
      if (action === "commit") {
        ...
        if (!committed) return;
        messages.success("✅ Committed!", "GitBuddy Branch");
      }

      if (action === "stash") {
        const stashed = await gitUtils.stash();
        if (!stashed) return;
        messages.success("📦 Changes stashed!", "GitBuddy Branch");
      }
    }
    ...
    console.log(
      boxen(
        chalk.green.bold("✅ Branch created successfully!\n\n") +
          ...
          chalk.gray("Run gitbuddy status to confirm."),
        {
          padding: 1,
          borderColor: "green",
          title: "🤖 GitBuddy Branch",
