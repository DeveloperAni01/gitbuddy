    ...
    try {
        ...
        if (repo.modified.length > 0 ||
            ...
            if (action === 'commit') {
                ...
                messages.success('✅ Committed! Now pulling...', 'GitBuddy Pull');
            }

            if (action === 'stash') {
                const stashed = await gitUtils.stash();
                if (!stashed) return;
                messages.success('📦 Changes stashed! Now pulling...', 'GitBuddy Pull');
            }
        }
        ...
        if (stashList && stashList.includes('stash@{0}')) {
            ...
            if (restore) {
                ...
                if (popped) {
                    messages.success('✅ Stashed changes restored!', 'GitBuddy Pull');
                }
            }
        }
