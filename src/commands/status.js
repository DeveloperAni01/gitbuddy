...
async function statusCommand() {
    try {
        ...
        // Ahead/behind remote
        if (repo.ahead > 0) {
            output += chalk.magenta(`🚀 Ahead of GitHub by ${repo.ahead} commit(s) — run gitbuddy push!\n`);
        }
        if (repo.behind > 0) {
            output += chalk.magenta(`⬇️  Behind GitHub by ${repo.behind} commit(s) — run gitbuddy pull!\n`);
        }
