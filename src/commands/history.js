...
async function historyCommand() {
    try {
        ...
        if (repo.ahead > 0) {
            ...
            output += chalk.gray('💡 Tip: Run gitbuddy push to sync!\n');
        }

        if (repo.behind > 0) {
            output += chalk.magenta(`⬇️  ${repo.behind} commit(s) on GitHub not pulled yet!\n`);
            output += chalk.gray('💡 Tip: Run gitbuddy pull to sync!\n');
        }
            ...
            if (action === 'more') {
                ...
                console.log(
                    boxen(moreOutput, {
                        padding: 1,
                        borderColor: 'cyan',
                        title: '🤖 GitBuddy History (continued)',
            ...
            if (action === 'oneline') {
                ...
                console.log(
                    boxen(onelineOutput, {
                        padding: 1,
                        borderColor: 'cyan',
                        title: '🤖 GitBuddy History (oneline)',
                        titleAlignment: 'center'
                    })
                );
            }
