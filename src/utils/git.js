    ...
    try {
        ...
        console.log(
            boxen(
                ...
                chalk.yellow('GitBuddy needs Git to work.\n\n') +
                chalk.white.bold('📥 Install Git:\n\n') +
                installInstructions +
                chalk.gray('\n\nAfter installing:\n') +
                chalk.gray('→ Restart your terminal\n') +
                chalk.gray('→ Run gitbuddy again ✅'),
                {
                    padding: 1,
                    borderColor: 'red',
                    title: '⚠️  GitBuddy Setup',
...
async function checkRepo() {
    try {
        ...
        if (error.message.includes('not a git repository')) {
            ...
            messages.error(
                '❌ Permission denied!\n\n' +
                'GitBuddy cannot access this folder.\n' +
    ...
    try {
        ...
        if (error.message.includes('rejected')) {
            messages.error(
                ...
                'Run gitbuddy pull first!\n' +
                'Then try gitbuddy push again.',
                'GitBuddy Error'
            );
    ...
    try {
        ...
        if (error.message.includes('conflict')) {
            messages.error(
                ...
                '  4. gitbuddy commit "resolved conflicts"',
                'GitBuddy Error'
            );
        } else if (error.message.includes('Authentication failed')) {
            messages.error(
            ...
            messages.error(
                ...
                'Run gitbuddy push to create it.',
                'GitBuddy Error'
            );
        } else {
            messages.gitError(error.message);
